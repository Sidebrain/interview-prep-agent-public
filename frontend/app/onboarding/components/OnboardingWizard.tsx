...
const createCandidate = async (
  ...
  const response = await fetch(
    ...
    }/api/v3/interview/candidate?${params.toString()}`,
    {
      method: "POST",
      headers: {
  ...
  try {
    const interviewSession: InterviewSession =
      InterviewSessionSchema.parse(await response.json());
...
const getInterviewer = async (interviewer_id: string) => {
  const response = await fetch(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v3/interview/${interviewer_id}`
  );
  if (!response.ok) {
    throw new Error("Failed to get interviewer");
  }
  try {
    const interviewer: Interviewer = InterviewerSchema.parse(
      await response.json()
    );
    return interviewer;
  } catch (error) {
...
const OnboardingWizard = ({
  ...
  const [stage, setStage] = useState<
    "interview_details" | "user_profile" | "provide_permissions"
  >("interview_details");
  ...
  const handleSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    const interviewSession = await createCandidate(
      candidateRequest,
      interviewer_id
    );
  ...
  const handlePermissionsGranted = () => {
    if (interviewSession) {
      router.push(
        `/interview-new?interview_session_id=${interviewSession._id}`
      );
    }
    console.log(interviewSession);
  };
  ...
  const renderStage = (
    stage: "interview_details" | "user_profile" | "provide_permissions",
    interviewer_id: string
  ) => {
    switch (stage) {
      case "interview_details":
        return (
          <InterviewDetails
            ...
            handleClick={() => setStage("user_profile")}
          />
        );
      case "user_profile":
