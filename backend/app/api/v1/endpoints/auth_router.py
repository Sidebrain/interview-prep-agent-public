...
async def firebase_auth_proxy(request: Request, path: str) -> Response:
    firebase_url = (
        f"https://public-project-436308.firebaseapp.com/__/auth/{path}"
    )
    ...
    response = await client.request(
        method, firebase_url, headers=headers, content=body
    )
